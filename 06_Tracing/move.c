 ...
 *  0  success
 *  1  args
 *  2  stat(infile) failed
 *  3  open(infile) failed
 *  4  fstat(infile) failed
 *  5  create/open temp failed
 *  6  read error
 *  7  write error
 *  8  fsync error
 *  9  close temp error
 * 10  rename error
 * 11  unlink(infile) failed (after successful rename)
 * 12  same file (infile == outfile)
 */
...
int main(int argc, char *argv[]) {
    ...
    /* If outfile exists and is same file as infile -> nothing to do */
    struct stat st_out;
    ...
    /* Build temp path in same directory as outpath */
    ...
    /* create temp file with same mode as infile */
    ...
    if (unlink(inpath) != 0) {
        ...
        /* outfile already replaced; report error code */
        close(fd_in);
        return EX_UNLINK_IN;
    }
